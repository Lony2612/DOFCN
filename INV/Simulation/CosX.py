...
class CosNet(nn.Module):
    def __init__(self, data_sigma, offset=0.0, omega=6.0, init_flag=False):
        ...
        if init_flag:
            ...
            nn.init.normal_(self.fc1.weight.data, 0.0, 0.006)
            nn.init.normal_(self.fc1.bias.data, 0.004, 0.01)
            
            nn.init.normal_(self.fc2.weight.data, 0.0007, 0.006)
            nn.init.constant_(self.fc2.bias.data, 0)

    def forward(self, x):
        x = self.conv1(torch.cos(x * self.omega))
        x = self.dp1(self.conv2(torch.cos(x * self.omega)))
...
model = CosNet(data_sigma=1.0, offset=25.0, omega=6, init_flag=True)
path = './models/Cos.pth.tar'
model.cuda()
