...
class XCosNet(nn.Module):
    ...
    def forward(self, x):
        offset = self.offset * 2 * np.pi * 0.02
        x = self.conv1(x * torch.cos(x*self.omega - offset))
        x = self.conv2(x * torch.cos(x*self.omega))
...
model = XCosNet(data_sigma=1.0, offset=25.0, omega=16)
path = './models/XCosX.pth.tar'
model.cuda()
