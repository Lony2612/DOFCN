...
class XCosXNet(nn.Module):
    def __init__(self):
        super(XCosXNet, self).__init__()
        self.pool1 = nn.MaxPool1d(kernel_size=2, stride=2)
        
        self.conv1 = nn.Conv1d(in_channels=2, out_channels=256, kernel_size=3, bias=False)
        self.conv2 = nn.Conv1d(in_channels=256, out_channels=80, kernel_size=3, bias=False)
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 4800)
        x = self.dropout3(self.lrelu3(self.fc1(x)))
...
def train(model_path='./models/XCosX.pth.tar', \
          loss_path='./logs/XCosX_train_loss.txt',\
          acc_path='./logs/XCosX_train_acc.txt'):
    ...
    try:
        for epoch in range(0, nb_epoch):
            ...
            for i, data in enumerate(loader):
                ...
                # forward+backward
                inputs = torch.reshape(inputs, [-1, 2, 128])
                ...
                if i % 100 == 0:
                    print(str([epoch+1, i+1])+' loss:'+str(running_loss/100))  # 平均损失函数值
...
def test(model_path='./models/XCosX.pth.tar', \
         acc_path='./logs/XCosX_test_acc.txt', 
    ...
    for snr in snrs:
        ...
        # estimate classes
        test_X_i = torch.reshape(test_X_i, [-1, 2, 128])
    ...
    plt.title("CNN Classification Accuracy on RadioML 2016.10 Beta")
    plt.close()
